...
### 3.5 HitRate@K
Ещё называют Success@K: «есть ли хотя бы одно попадание в топ-K».

#### Формулы

Пусть $\mathbb{1}_{r_{u j}}$ — индикатор того, что пользователь $u$ **взаимодействовал** с айтемом $j$ (то есть $j$ релевантен для $u$ в тесте).

Тогда для топ-K рекомендаций $\pi_u=[i_1,\dots,i_K]$:

$$
HitRate@K(u) = \max_{pos \in [1..K]} \mathbb{1}_{r_{u i_{pos}}}
$$

То есть:

* **1**, если среди топ-K есть хотя бы один релевантный айтем,
* **0**, если нет.

И усреднение по пользователям:

$$
HitRate@K = \frac{1}{|U|}\sum_{u\in U} HitRate@K(u)
$$

#### Смысл

* Это метрика «хоть что-то угадали для пользователя в топе».
* Практически эквивалентна **Recall@K**, если для каждого пользователя в тесте ровно **1** релевантный айтем ($|Rel(u)|=1$):
  $$
  HitRate@K(u)=Recall@K(u)\quad \text{при } |Rel(u)|=1
  $$
...
* Если релевантные в тесте: ${D}$ → HitRate@5(u)=1 (попали).
* Если релевантные в тесте: ${F}$ → HitRate@5(u)=0 (не попали).

#### Ограничения

* Не различает «1 попадание» и «5 попаданий» (оба дают 1).
* Не различает попадание на 1-й и на K-й позиции.
* Поэтому часто используют как простую метрику для **next-item/поисковых** сценариев или когда нужен быстрый sanity-check.

---

### 3.6 Coverage@K

Coverage измеряет, какую долю каталога (или допустимого множества айтемов) модель **вообще** умеет показывать, если посмотреть на рекомендации всем пользователям.

#### Формула

Пусть $y_u={i_1,\dots,i_K}$ — множество айтемов, рекомендованных пользователю $u$ (топ-K без учёта порядка).

$$
Coverage@K = \frac{\left|\bigcup\limits_{u\in U} y_u\right|}{|I|}
$$

Где $I$ — множество всех айтемов (на практике часто берут «все recommendable»: после фильтрации по наличию, бан-листов, минимальной популярности и т.п.).

#### Смысл

* Высокий coverage → модель не «залипает» на узком подмножестве хитов, а распределяет показы по более широкой части каталога.
* Это оффлайн-прокси для **diversity/catalog exposure** (но не то же самое, что intra-list diversity).

#### Мини-пример

Каталог $|I|=10$.

* Если для всех пользователей топ-5 всегда из одних и тех же 5 айтемов → $Coverage@5=5/10=0.5$.
* Если в сумме по всем пользователям в топ-5 встречаются 9 разных айтемов → $Coverage@5=0.9$.

#### Важные замечания

* Coverage легко "накрутить" шумом/рандомом, поэтому его смотрят **вместе** с метриками точности (nDCG/MAP/Recall).
* Часто полезно дополнительно смотреть coverage по срезам: только long-tail айтемы, только новые айтемы, только товарные категории.

---

### 3.7 Когда какую метрику использовать
...
* **Top-K ranking, бинарная релевантность (implicit)**:
  ...
  * «качество всего топа + ранние позиции важнее» → **MAP@K** или **nDCG (binary rel)**.
  * «важно хотя бы одно попадание в топ-K» → **HitRate@K** (успех/неуспех), иногда вместе с **MRR**.
  * «важно первое раннее попадание» → **MRR** (поиск, next-item, похожие объекты).
  * «чистота топ-k» → **Precision@K** (простая и интерпретируемая, но не видит порядок внутри топа).
* **Candidate generation / retrieval** (первый этап пайплайна): **Recall@K** (K ≫ финального, например 50/100/200), т. к. важно не потерять релевантное.
* **Контроль “узости” рекомендаций / экспозиции каталога**: **Coverage@K** (в паре с точностными метриками).
