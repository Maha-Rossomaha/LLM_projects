...
## 6. Почему SLIM и EASE до сих пор используются и какие у них проблемы

**SLIM** (Sparse Linear Methods) и **EASE** (Embarrassingly Shallow AutoEncoder) — это линейные item–item модели, которые предсказывают интерес к айтему как линейную комбинацию взаимодействий пользователя с другими айтемами: $\hat R \approx R W$ при разреженной/регуляризованной $W$. Несмотря на “возраст”, их до сих пор держат в проде, потому что они закрывают важный класс задач дешевле и надёжнее многих сложных моделей.

#### Почему их продолжают использовать

1) **Сильный baseline и “lower bound” для качества**  
На implicit данных SLIM/EASE часто дают очень приличный nDCG/MAP и служат эталоном: если сложная модель не обгоняет их, значит либо фичи/логика плохие, либо обучение/оценка сделаны неправильно.

2) **Отличный retrieval / генерация кандидатов**  
Их типичная роль — быстро дать набор кандидатов (например, top-100/200) с хорошим recall. Ранжировщик потом “дожимает” контекстом и бизнес-логикой. В этом месте важны скорость, устойчивость и простота — всё это у них есть.

3) **Простота, предсказуемость, интерпретируемость**  
Вес $w_{ji}$ можно интерпретировать как "item $j$ поддерживает предсказание $i$". Это упрощает дебаг: легко понять, почему модель рекомендует конкретный товар и где она ломается.

4) **Дешёвое обучение и обслуживание**  
EASE обучается фактически решением линейной задачи/замкнутой формой (в упрощённом виде), SLIM — оптимизацией с разреживанием. В обоих случаях это проще, чем нейросети, меньше зависимостей, меньше точек отказа.

5) **Хорошая устойчивость на практике**  
Они редко “взрываются” из-за смены распределения или проблем с фичами, потому что опираются на базовый сигнал: user–item взаимодействия и item–item связи.

---

#### Основные проблемы и ограничения

1) **Нет контекста (и это фундаментально)**  
SLIM/EASE по сути стационарны: они не знают время, цену, скидку, регион, устройство, сезонность, намерение пользователя. Поэтому в роли финального ранжировщика почти всегда уступают LTR/нейросетям, которые могут учесть контекстные фичи.

2) **Feedback loop и популярность**  
Модели легко “залипают” на популярном: показали → кликнули → усилили связи → ещё показываем. Без специальных мер (reweighting, debiasing, регуляризация, бизнес-ограничения) они тянут рекомендации в head и ухудшают разнообразие.

3) **Оффлайн хорошо, онлайн не гарантировано**  
Они оптимизируют прокси (восстановление взаимодействий, nDCG/MAP), но бизнес-цели могут быть другими (CTR/конверсия/GMV/retention). В онлайне важны позиционный bias, задержки, калибровка, разнообразие — линейная модель без контекста это покрывает слабо.

4) **Слабая адаптация к изменениям**  
Интересы и ассортимент меняются быстро, а SLIM/EASE обычно переобучаются пакетно. Без инкрементальных обновлений или гибридизации с более “живыми” сигналами они устаревают.

5) **Проблемы холодного старта**  
Новый айтем без взаимодействий не имеет надёжных весов в $W$. Новый пользователь без истории тоже почти не обслуживается. Обычно это решают гибридом: контентные эмбеддинги/атрибуты, популярность, правила, exploration.

6) **Стоимость по памяти/хранению для больших каталогов**  
Хотя $W$ делают разреженной (top-k соседей), при миллионах айтемов хранение и обновление item–item графа всё равно нетривиально. Это не “бесплатно”, просто дешевле, чем многие альтернативы при сопоставимом качестве на этапе retrieval.

---

#### Практический вывод
В больших системах SLIM/EASE чаще всего живут как **retrieval-слой / baseline / fallback / один из сигналов ансамбля**, потому что дают стабильное качество при низкой стоимости. Их ключевой потолок — **отсутствие контекста и дебайзинга**, поэтому как единственный финальный ранжировщик они обычно не выдерживают требования продовых метрик и ограничений.