# Матричная факторизация (Matrix Factorization, MF) в рекомендательных системах
...
- `\Sigma ∈ ℝ^{m×n}` — диагональная (в прямоугольном смысле) матрица сингулярных чисел `σ_1 ≥ σ_2 ≥ … ≥ 0`.
...
**Почему “шум” может быть полезен** (твоя интуиция про картинки → рекомендации):
...
Практический смысл:
- если `V_k,\Sigma_k` зафиксированы, можно быстро получить **вектор нового пользователя** по его строке `m_u` (взаимодействия с айтемами):
...
- Регуляризация (`\lambda`) борется с переобучением.
...
Обозначим:
- `I_u = {i: (u,i)∈Ω}` — айтемы, которые оценил пользователь.
...
## 13) Вопросы для интервью (самопроверка)
1) Чем model‑based CF отличается от memory‑based?
2) Почему MNAR важен и какие bias он создаёт?
3) Почему Truncated SVD — лучшее rank‑k приближение? Что такое `σ_i`?
4) Почему заполнение пропусков нулями может быть вредным?
5) Что такое folding‑in и что он **не** делает?
6) В чём отличие FunkSVD от “SVD с нулями”?
7) Зачем bias (`μ, b_u, b_i`) и как они помогают?
8) Что добавляет SVD++ и почему там есть `|R(u)|^{-1/2}`?
9) Почему ALS хорошо параллелится? Что именно решается на шаге?
10) В iALS зачем нужны `p_{ui}` и `c_{ui}`? В чём вычислительный трюк `Y^T Y`?
11) Почему dot product может давать popularity bias?
12) Как бы ты сделал item2item и персональный retrieval на эмбеддингах?
