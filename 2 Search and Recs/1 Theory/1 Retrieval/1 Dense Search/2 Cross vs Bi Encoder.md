# Cross vs Bi-encoder (и Twin-tower)
...
* **Twin‑tower (two‑tower)**: запрос и документ кодируются разными энкодерами \$q = f\_q(y)\$, \$d = f\_d(x)\$. Это гибче: query‑tower можно оптимизировать под короткие тексты, doc‑tower — под длинные. Но требует больше параметров и обучения.
* **Cross‑encoder**: формируем пару $\[y; x]\$ и подаём её целиком в модель; она сразу выдаёт \$score = f\_{cross}(\[y; x])\$. Качество выше всего, но инференс дорогой.
...
* \$q = f\_q(y)\$, \$d = f\_d(x)\$, энкодеры разные.
...
* **Bi‑encoder**: offline считаем все \$d\_i\$, online: кодируем \$q\$, ищем через ANN.
