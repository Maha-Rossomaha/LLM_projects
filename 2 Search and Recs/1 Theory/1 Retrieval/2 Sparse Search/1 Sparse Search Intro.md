...
$$
score_{BM25+}​(d,q)=\sum_{t\in q}​IDF(t)\cdot\frac{f(t,d)\cdot(k+1)}{f(t,d)+k\cdot(1−b+b\cdot\tfrac{∣d∣}{avgdl})+\delta\cdot IDF(t)}
$$
...
$$
TF_{BM25L​}(t,d)=\frac{f(t,d)}{f(t,d)+k\cdot((1−b)+b\cdot\frac{∣d∣}{avgdl})+\epsilon}
$$
$\;\;\;\;\;\;\;\;$где $\epsilon$ — малое положительное число (обычно $0.25$).
