...
**BM25 (классическая):**  
$$
    score(q,d) = \sum\limits_{t \in q} idf_t \cdot \frac{tf_{t,d} \cdot (k_1 + 1)}{tf_{t,d} + k_1 \cdot \left(1 - b + b \cdot \frac{|d|}{avgdl}\right)}.
$$

**IDF (с сглаживанием Робертсона):**  
$$
...
4. **Адаптивные стоп‑списки**: автопрунинг термов с $df_t/N > \tau_{hi}$; и/или агрессивное фильтрование очень редких мусорных токенов ($df_t/N < \tau_{lo}$).
...
**Пересчёт $idf_t$ и $avgdl$ обязателен, если** выполнено одно из условий:
