# Дизайн эксперимента для Uplift
...
### 4.1. Задача
- Убедиться, что распределение признаков в Treatment и Control похоже (до эффекта). 
...
#### 4.3.1 Standardized Mean Difference (SMD)

Для числового признака $X$:

$$
\text{SMD} = \frac{\bar X_T - \bar X_C}{s_{\text{pooled}}}, \quad 
s_{\text{pooled}} = \sqrt{\frac{s_T^2 + s_C^2}{2}}
$$

- $\bar X_T, \bar X_C$ — средние в группах.
- $s_T, s_C$ — стандартные отклонения в группах.
- По сути: **разность средних, измеренная в "$\sigma$"** (сколько стандартных отклонений между группами).

Правило пальца:
- $|\text{SMD}| < 0.1$ → баланс хороший.  
- 0.1–0.2 → терпимо, но уже настораживает.  
- $|\text{SMD}| > 0.2$ → явный дисбаланс.

Для бинарных признаков вместо средних берутся доли:
$$
\text{SMD} = \frac{p_T - p_C}{\sqrt{\frac{p_T(1-p_T)+p_C(1-p_C)}{2}}}
$$

#### 4.3.2. Гистограммы / ECDF / KS-тест для непрерывных признаков

Идея: сравниваем **распределения** признака в Treatment и Control.

- **Гистограммы / KDE**:  
  На один график кладём два распределения. Глазами смотрим:
  - совпадает ли форма (симметрия, хвосты),
  - нет ли сдвига по центру,
  - нет ли сильных хвостов в одной группе.

- **ECDF (Empirical CDF)**:  
  Для каждой группы строим эмпирическую функцию распределения $F_T(x), F_C(x)$:  
  по оси $X$ значение признака, по оси $Y$ доля наблюдений $\le X$.  
  - Если кривые почти совпадают → хороший баланс.
  - Если расходятся систематически → проблема.

- **KS-тест (Kolmogorov–Smirnov)**:
  - Статистика: $D = \sup_x |F_T(x) - F_C(x)|$ — максимальная вертикальная разница между ECDF.
  - $H_0$: распределения одинаковы.
  - Малое p-value → распределения различаются.
  - На больших выборках KS становится сверхчувствительным: может “находить” разницу там, где по бизнесу она неважна, поэтому **смотрим и на графики, и на SMD**, а не молимся на p-value.

#### 4.3.3. Доли / $\chi^2$ для категориальных признаков

Для категориального признака (например, сегмент / регион) строим **таблицу сопряжённости**:

| Категория | Control count | Treatment count |
|-----------|---------------|-----------------|
| A         | $n_{c_A}$     | $n_{t_A}$       |
| B         | $n_{c_B}$     | $n_{t_B}$       |

- **$\chi^2$-тест**:
  - $H_0$: распределение категорий одинаковое в группах.
  - Статистика $\chi^2$ измеряет, насколько наблюдаемые частоты расходятся с ожидаемыми при "нет различий".
  - Малое p-value → признак распределён по группам по-разному (дисбаланс).
- Для редких категорий / маленьких чисел уместнее Fisher exact, но в uplift-практике обычно хватает $\chi^2$ + здравый смысл (объединить хвост в “Other”).

#### 4.3.4. Love-plot (сводный график SMD по фичам)

График для визуализации баланса **по всем признакам сразу**.

Как выглядит:
- По оси $Y$: список признаков.
- По оси $X$: SMD для каждого признака.
- Часто рисуют **две серии точек**:
  - до матчинга/весов (raw),
  - после (adjusted).

Интерпретация:
- Видим, сколько признаков выбиваются за $|\text{SMD}| > 0.1$.
- Хороший сценарий:  
  - "До" точки разлетаются, многие > 0.2;  
  - "После" почти все лежат в коридоре $[-0.1, 0.1]$.
- Это наглядный способ продемонстрировать, что применённые методы корректировки (matching / взвешивание по propensity score) действительно улучшили баланс ковариат между группами, а не остались формальной процедурой.

### 4.4. Что делать при дисбалансе
- Убедиться, что он случаен и мал; при сильном — применить **стратифицированный анализ**, регрессионную поправку (CUPED/ANCOVA) или перезапустить рандомизацию с блоками.
...
- Нужны **достаточные события** в каждой ветви (W=0/1). Правило: ≥ **1000–2000 положительных исходов** per arm для устойчивого обучения/валидации. Иначе оценки uplift и метрики (Qini, AUUC) становятся сильно шумными, модель “ловит мусор”, а результаты эксперимента легко переворачиваются при малейшем изменении выборки.
- Доля W=1 близка к 50% даёт минимальную дисперсию разности и лучшую наполняемость бинов в Qini. При сильном перекосе (например, 10/90) растёт дисперсия оценки эффекта, часть сегментов остаётся почти без контроля или таргета, и любые выводы по ним становятся ненадёжными.
- Для редких событий (p<1%) планируйте **длительный** сбор или агрегацию по кластерам. При слишком коротком горизонте или мелких сегментах число событий просто не успевает набраться, и uplift-модель/кривые Qini неотличимы от случайного шума.
...
### 6.3. Bandits vs RCT
- Многорукие бандиты минимизируют regret онлайн, но **сложнее** для чистой каузальной оценки и Qini. Рекомендуется: сначала RCT→обучение→политика; затем уже bandit‑эксперименты для оптимизации.
