# Обзор функций активации в трансформерах и LLM
...
- GELU по сути говорит:
> Пропускаем $x$ в $output$ с вероятностью, зависящей от того, насколько он вероятен в нормальном распределении
- Это приближает его к стохастическому нейрону, но детерминированно
...
$$
\text{SwiGLU}(xW_1, xW_2) = \text{SiLU}(xW_1) \cdot (xW_2)
$$

**Смысл:** 
- Eсть два линейных проектора:
  - один — под гейт (через SiLU)
  - второй — под основной поток данных
- После поэлементного перемножения получаем gated activation, где:
  - Один поток ($W_1$) решает, насколько сильно активировать каждый элемент
  - Второй поток ($W_2$) — какое значение передать
...
**Особенность:** Увеличивает количество параметров FFN в полтора раза (из-за дублирования проекции)
