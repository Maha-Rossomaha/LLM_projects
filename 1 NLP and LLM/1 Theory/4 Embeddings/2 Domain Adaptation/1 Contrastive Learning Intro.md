...
Embedding space может начать учитывать только лексическое сходство, игнорируя смысл. Например, "Как приготовить курицу" и "Как приготовить индейку" могут быть ближе друг к другу, чем "Как загрузить курицу в Blender" (3D-программа).
...
$$
L = -\log\left( \frac{\exp(\text{sim}(q, d^+))}{\exp(\text{sim}(q, d^+)) + \sum_j \exp(\text{sim}(q, d_j^-))} \right)
$$
...
* Является парным случаем InfoNCE с одним негативом
