...
Фиксируем суммы. Тогда $A\sim\text{Hypergeom}(N=20, K=7, n=10)$. Это буквально: «вытащили 10 человек из 20, среди которых 7 успешных, какая вероятность что среди этих десяти будет k успехов».

Считаем вероятности для «левых хвостов» (мало успехов в A):
...
где $P(A=0)$ - более «экстремальное в ту же сторону»
...
Разница большая на глаз (1/10 vs 6/10), но выборки микроскопические.  
Тест говорит буквально следующее:
* Если ты заранее сформулировал гипотезу «B лучше A», то данных хватает, чтобы это поддержать (односторонний тест).
* Если формулировка была общая «доли различаются», то данных уже не хватает, потому что двусторонний тест строже и штрафует за то, что рассматривает симметричную альтернативу тоже.  
  
Это и есть «граничный кейс»: эффект очевиден, но статистическая мощность маленькая.
...
- В отчёте показывать: таблицу 2×2, $p$‑value, OR с точным CI, а также абсолютную/относительную разницу долей.
...
```
A: 1/10 = 0.100;  B: 6/10 = 0.600
Absolute diff = 0.500; Relative lift = 500.00%
Fisher two-sided p-value = 0.0573 | one-sided: less=0.0286, greater=0.9985
Odds Ratio (OR) = 0.074 with exact CI [0.007, 0.835]
Risk Ratio (RR) = 0.167 with CI [0.024, 1.145]
```
