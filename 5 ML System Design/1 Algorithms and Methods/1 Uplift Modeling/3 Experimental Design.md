...
**Цель:** определить корректный вид данных и протокол, при котором инкрементальные эффекты идентифицируемы и поддаются честной оценке.

> Терминология: *воздействие* (treatment) — факт назначения коммуникации/оффера/лимита/показа. Мера эффекта — ATE/CATE/uplift. Единица рандомизации — пользователь/счёт/домохозяйство/кластер.

---

## 0) Коротко
- **Что хотим?** Оценить и выучить \(\tau(x)=\mathbb{E}[Y(1)-Y(0)\mid X=x]\), чтобы таргетировать тех, у кого прирост от воздействия положительный и значимый.
- **Что требуется?** Рандомизация (или правдоподобная её имитация), контрольная группа, проверка баланса и достаточная мощность.
- **Чего избегаем?** Назначения «по склонности», пост‑тритмент переменных, контаминации и интерференции.
...
## 10) Чек‑лист запуска
1. Зафиксируй **W, единицу, окно, метрики, первичный эффект**.
2. Выбери вид рандомизации (простая/блочная/кластерная), долю W=1.
3. Проведи **power analysis** (включи запас на некомплаенс). 
4. Настрой **логирование** (см. список) и систему исключения пересечений.
5. Проверь **баланс** на pre‑period, запусти **dry‑run**.
6. Определи **правила остановки/анализов** (sequential/глобальный срок), не подглядывай без корректировок.
7. Заморозь holdout для честной оценки политики/модели.

---

## 11) Резюме
- Лучший источник данных для uplift — **RCT** с достаточной долей W=1, корректно определённым W и строгим логированием.
- При observational данных — **propensity/DR** и агрессивная диагностика баланса/оверлапа.
- Проверяй **баланс**, **мощность**, **контаминацию** и **интерференцию**; проектируй эксперимент с учётом реальных ограничений каналов и стоимости.
