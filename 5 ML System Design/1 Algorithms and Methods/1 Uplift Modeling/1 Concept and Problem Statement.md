# Uplift Modeling — Концепция и постановка задачи

> **Терминология:** далее вместо *treatment* используем слово **воздействие** (коммуникация/оффер/кампания/назначение терапии).
...
3. Для каждого префикса (топ-q%) вычисляем **накопленный инкремент**:
   ...
   $$
   где $R_{t,b}$, $R_{c,b}$ — число ответов в группе воздействия и контроля в бине $b$, $N_{t,b}$, $N_{c,b}$ — их размеры. Масштабирующий множитель $\tfrac{N_{t,b}}{N_{c,b}}$ компенсирует неодинаковые размеры.
...
## 6. Комментарии по допущениям и границам

- Этот конспект намеренно не уходит глубоко в допущения (SUTVA, ignorability, overlap), дизайн эксперимента (баланс, power, A/B/n), и методы «очистки» наблюдательных данных (propensity, matching, IPTW, doubly robust). Всё это — в следующих конспектах.
- Для **небинарного воздействия** (доза/ставка) и **небинарного таргета** (регрессия) uplift обобщается: моделируем разницу ожидаемых исходов между уровнями воздействия или его производную по интенсивности.

---

## 7. Мини-кейсы (интуиция)
...
### 7.2. Персональное кредитное предложение (КСП)
...
## 8. Частые ошибки и анти‑паттерны
