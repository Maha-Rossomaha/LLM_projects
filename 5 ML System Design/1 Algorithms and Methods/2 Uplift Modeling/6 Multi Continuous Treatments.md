...
### 3.1. Объект: функция dose–response (зависимость результата от размера воздействия)

Рассматриваем непрерывное воздействие $T \in R$ — это может быть доза лекарства, величина процентной ставки, размер скидки, объём маркетинговых расходов и т.п.

Для каждой потенциальной дозы $t$ существует соответствующий потенциальный исход $Y(t)$ — значение метрики при условии, что объект получил дозу $t$.

Три основных объекта исследования:

- **Функция dose–response:**  
  Она описывает ожидаемый результат при дозе (воздействии) $t$ для объектов с признаками $X=x$
  $$
  \mu(t,x) = \mathbb E[Y(t) \mid X=x];
  $$
  То есть это гладкая кривая зависимости эффекта от дозировки, уже очищенная от смешивающих факторов.

- **Маржинальный эффект (производная результата по дозировке):**  
  
  $$
  \theta(t,x) = \frac{\partial}{\partial t} \mu(t,x);
  $$
  **Интерпретация:** насколько изменится ожидаемый результат, если увеличить дозировку на бесконечно малую величину в точке $t$.  
  Это прямой аналог градиента в эконометрике и политике интервенций.

- **Индивидуально оптимальная дозировка:**
...
**Интерпретация:** насколько *«типична»* доза $t$ для объекта с характеристиками $x$.
* Если в данных клиент с данными $x$ обычно получает дозу около $t$, то $r(t,x)$ высоко.  
* Если такую дозу ему почти никогда не дают, то $r(t,x)$ низок.

**Свойства:**
* если контролировать за $r(T,X)$, то распределение treatment становится «почти случайным» внутри уровней GPS;
* следовательно, можно корректно оценивать зависимость результата от дозировки.

Идея аналогична бинарному **“conditional independence assumption”**, только для плотности, а не вероятности.

#### **Процедура построения**
#### 1. Моделирование условной плотности $f_{T\mid X}$  
Нужно оценить, как в данных выбиралась дозировка. Это аналог логистической регрессии для бинарного treatment, только теперь модель выдаёт плотность.  
Типичные модели:  
  * **нормальная регрессия:** $T \mid X = x \sim \Nu(\mu(x), \sigma^2)$;
  * гамма / лог-нормаль, если дозировки неотрицательные;
  * любой вероятностный ML (GAE, normalizing flows, mixture density networks), если распределение сложное.  
  
#### 2. Вычисление оценки GPS  
Для каждого наблюдения подставляются фактическая (наблюдаемая) доза и признаки в модель:  
$$
  \hat r_i = \hat f _{T\mid X}(T_i\mid X_i).
$$ 
 
#### 3. Использование GPS для восстановления dose–response
##### 3.1 Подход 1: Регрессия результата на $T$ и $\hat r$  
Обычно берут модель вида
$$
  Y = g(T, \hat r) + \varepsilon,
$$  
где $g$ — гибкая нелинейная модель (сплайны, полиномы, градиентные бустинги, нейросети).

**Как работает:**

* фиксируем признаки $x$,
* подаваем в модель различные значения дозировки $t$,
* можно восстановить $\mu(t,x)$.

> **Основная идея** - мы не обучаем на дискретных бинах, мы восстанавливаем (интерполируем) функцию $\mu$. Treatment остается непрерывным значением. 

Получив модель $g$, мы делаем $n$ скорингов каждой записи с разными значениями $t$, строим кривую 
$$
  t \mapsto \mu(t, x)
$$
и выбираем оптимальный $t$:
$$
  t^*(x) = \argmax_t\mu(t,x)-Cost(t,x)
$$

Этот метод — основной и самый гибкий.

##### 3.2 Стратификация по GPS

Алгоритм:

1. Разбить диапазон $T$ на интервалы $bins$.
2. Разбить диапазон GPS $\hat r$ на интервалы.
3. Рассмотреть «ячейки» вида
$$
  T \in \text{bin}_i,\\
  \hat r \in \text{bin}_j.
$$
4. Внутри каждой ячейки сравнивать исходы — там treatment условно случайный - $\hat\mu(b_i,c_i)=\mathbb{E}[Y\mid T\in \text{bin}_i, r\in\text{bin}_j]$.
5. Усреднять локальные оценки для восстановления функции $\mu(t,x)$.

**Шаги для восстановления $\mu(t,x)$:**  

**Шаг 1: усреднение по уровням GPS**  

GPS-разбиение — служебное. Оно нужно, чтобы получить unbiased локальные оценки.

Чтобы восстановить зависимость от $T$, надо «коллапсировать» GPS-измерение:
$$
  \hat\mu(b_i)=\sum_{j=1}^m\hat\mu(b_i,c_j)\cdot w_{ij}
$$
где веса $w_{ij}$:
* пропорциональны числу наблюдений,
* или пропорциональны плотности GPS,
* или равномерные.  

Так получаются $n$ точек: 
$$
  (b_1, \hat\mu(b_1)), (b_2, \hat\mu(b_2)), ..., (b_n, \hat\mu(b_n))
$$
Это дискретная аппроксимация гладкой функции **dose–response**.  

**Шаг 2: сглаживание/аппроксимация функции**  

Используются стандартные методы сглаживания:
* локальная регрессия (LOESS/LOWESS),
* сплайны,
* кусочно-линейная регрессия,
* монотонные сплайны (если знаем монотонность),
* Гауссовские процессы,
* нейросети с одним входом $t$,
* линейная интерполяция между точками.

**Шаг 3: для каждого конкретного клиента x?**  

Стратификация по GPS — не персонализированная модель, она оценивает $\mu$(t)$, а не $\mu(t,x)$.

То есть: 
* это popuplation-level dose–response, не CATE.
* персональные кривые $\mu$(t,x)$ не восстанавливаются.

Если нужен индивидуальный эффект, стратификация НЕ подходит.
...
- поддерживает различные задачи, бинарный / continuous treatment.
