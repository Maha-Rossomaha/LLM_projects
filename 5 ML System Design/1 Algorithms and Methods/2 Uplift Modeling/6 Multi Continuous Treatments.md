...
* подавая в модель различные значения дозировки $t$,
* фиксируя признаки $x$,
...
$$
  T \in \text{bin}_i,; \hat r \in \text{bin}_j.
$$
4. Внутри каждой ячейки сравнивать исходы — там treatment условно случайный.
5. Усреднять локальные оценки для восстановления функции $\mu(t,x)$.

**Плюсы:** простота, высокая интерпретируемость.  
**Минусы:** теряется точность, особенно при непрерывном treatment.
