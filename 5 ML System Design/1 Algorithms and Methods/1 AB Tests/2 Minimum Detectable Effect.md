...
## 1) Базовые определения и интуиция

**Alpha (α)** — вероятность ошибки I рода: отклонить H0, когда эффекта нет (ложная тревога). Часто 0.05 (двусторонний тест).
**Beta (β)** — вероятность ошибки II рода: не обнаружить реальный эффект.
**Power (1−β)** — вероятность **обнаружить** эффект заданной величины (целимся в 0.8 или 0.9).
**MDE (Minimum Detectable Effect)** — минимальный эффект (разница), который эксперимент должен уметь обнаружить с заданными α и power.

Интуиция:

* Чем **меньше MDE** и/или **меньше шум (дисперсия)** — тем **меньше** нужен объём.
* Чем **жёстче α** (меньше) и/или **выше power** — тем **больше** нужен объём.
* Для бинарных метрик шум ~ `p(1−p)` (максимум при p≈0.5).
* Для средних — шум ~ дисперсия метрики (σ²). Снижение σ² (CUPED, стратификация) уменьшает N.
...
**Модель.** Пусть pA — базовая конверсия, pB — pA + Δ.
**MDE (абсолютный)**: Δ_abs = pB − pA.
**MDE (относительный)**: Δ_rel = (pB − pA) / pA. Тогда pB = pA · (1 + Δ_rel).

**Эффект-сайз (Cohen’s h)** для пропорций:
h = 2·arcsin(√pB) − 2·arcsин(√pA).
...
**Модель.** μA и μB — средние, σ — std (если дисперсии не равны — Welch, но в планировании часто берут «типичную» σ).
**Эффект-сайз (Cohen’s d):** d = (μB − μA)/σ.
...
* Бинарная: pA = 0.10, хотим Δ_rel = +5% ⇒ pB = 0.105 ⇒ Δ_abs = +0.005 (0.5 п.п.).
* Непрерывная: μA = 100 ₽, MDE_abs = +3 ₽; если σ=30 ₽ ⇒ d = 3/30 = 0.1.
...
def n_per_group_for_proportions(p_baseline: float,
    ...
    """
    Вернёт размер выборки на группу (для контрольной). Для тестовой — умножить на ratio.
    """
    ...
    n_ctrl = analysis.solve_power(effect_size=effect_size,
                                  alpha=alpha,
                                  power=power,
                                  ratio=ratio,
...
nA = n_per_group_for_proportions(p_baseline=0.10, mde_relative=0.05, alpha=0.05, power=0.8)
nB = nA  # ratio=1
print(nA, nB)
```
...
* При A/B/n скорректируйте α (Bonferroni/Holm) или проводите подтверждающий A/B для победителя — это меняет N.
...
def n_per_group_for_means(mu_baseline: float,
                          mde_abs: float,
                          sigma: float,
                          alpha: float = 0.05,
                          power: float = 0.8,
                          ...
                          alternative: str = "two-sided") -> float:
    """
    Предполагаем независимые выборки. Используем Cohen's d = delta / sigma.
    """
    d = mde_abs / sigma
    analysis = TTestIndPower()
    n_ctrl = analysis.solve_power(effect_size=d,
                                  alpha=alpha,
                                  power=power,
                                  ratio=ratio,
                                  alternative=alternative)
...
nA = n_per_group_for_means(mu_baseline=100, mde_abs=3, sigma=40, alpha=0.05, power=0.8)
print(nA)
```

**Если распределение сильно правохвостое (логнормаль):**

* Планируйте тест на **логах**: MDE задавайте как относительный множитель (например, +3%), эквивалентно разнице средних логов ≈ ln(1.03).
...
Пусть нужно `nA` и `nB` пользователей. Ежедневный «видящий» трафик — DAU_testable. Доли трафика: wA и wB (обычно 50/50).

Оценка дней:
days ≈ max(nA / (DAU_testable·wA), nB / (DAU_testable·wB)).

```python
import math

def estimate_days(nA: int, nB: int, dau_testable: int, wA: float = 0.5, wB: float = 0.5):
...
print(estimate_days(60_000, 60_000, 80_000, 0.5, 0.5))  # ~2 дня (идеально), на практике заложите буфер
```

**Компромиссы:**

* Меньший MDE ⇒ больше N ⇒ дольше.
* Больше power или меньше α ⇒ больше N.
...
Шум Var ≈ p(1−p). При p=0.5 максимален ⇒ нужен больший N. Если p очень мало/много, дисперсия меньше ⇒ N меньше.

```python
import numpy as np

def sweep_n_vs_baseline(mde_rel=0.05, alpha=0.05, power=0.8):
    ps = np.linspace(0.02, 0.5, 25)
    ns = []
    for p in ps:
        ns.append(n_per_group_for_proportions(p_baseline=p, mde_relative=mde_rel, alpha=alpha, power=power))
    return ps, ns
...
### 6.2. Непрерывная: роль σ

N зависит от d = Δ/σ. При фиксированном Δ уменьшение σ в 2 раза снижает N примерно в 4 раза (квадратичный эффект).
**Варианты снижения σ:**
...
Design Effect (DE) ≈ 1 + (m − 1)·ICC, где m — средний размер кластера.
Эффективный объём = N_сырые / DE ⇒ чтобы сохранить power, **умножаем** N на DE.

```python
def inflate_by_design_effect(n_raw: int, m: float, icc: float) -> int:
    deff = 1 + (m - 1) * icc
    return math.ceil(n_raw * deff)

print(inflate_by_design_effect(50_000, m=5, icc=0.02))  # пример
```
...
Если число событий на пользователя ~ Пуассон(λ), можно планировать по **отношению скоростей** (rate ratio). При экспозиции T на пользователя Var(λ̂) ≈ λ/T.

Упрощённый подход: перейти к **непрерывной метрике «события на пользователя»** и применить блок 4.2, оценив σ по пилоту. Для редких событий часто лучше агрегировать **по пользователю** за окно, тогда t-тест по средним работает приемлемо.

### 7.2. Сильно правохвостые метрики (выручка)

* Планируйте тест на **лог-метрике** (или используйте винзоризацию/обрезку хвостов).
* В расчёте N берите σ лог-метрики; MDE задавайте как относительный множитель (например, +3% ⇒ delta_log ≈ ln(1.03)).
...
# 1) Найти N на группу
n = analysis.solve_power(effect_size=h, alpha=0.05, power=0.8, ratio=1.0, alternative="two-sided")

# 2) Посчитать power при фиксированном N
power = analysis.power(effect_size=h, nobs1=n, alpha=0.05, ratio=1.0, alternative="two-sided")
...
targets = np.linspace(0.002, 0.02, 10)  # сетка абсолютных MDE
```
...
tt = TTestIndPower()
d = 0.1  # Cohen's d
n = tt.solve_power(effect_size=d, alpha=0.05, power=0.8, ratio=1.0, alternative="two-sided")
pw = tt.power(effect_size=d, nobs1=n, alpha=0.05, ratio=1.0, alternative="two-sided")
```
...
* **A/B/n:** если вы сразу тестируете несколько вариантов, эффективный α на сравнение снижается (Bonferroni/Holm), что **увеличивает N**. Альтернатива — отбор победителя и подтверждающий A/B (двухэтапный процесс).
* **«Подглядывание» (peeking):** преждевременная остановка и цикличные проверки искажают α/p. Если нужно раннее решение — используйте **групповые последовательные дизайны** или байес-мониторинг (это отдельная тема). На этап планирования закладывайте фиксированную длительность.
...
# 2) N
nA = n_per_group_for_proportions(p_baseline=pA, mde_relative=mde_rel,
...
nA = n_per_group_for_means(mu_baseline=0.0,  # не используется
                           mde_abs=delta_log,
                           sigma=sigma_log,
...
1. **Оцените σ или baseline p по пилоту** (или по ретроспективным данным), чтобы план был реалистичным.
2. **Фиксируйте параметры до старта**: α, power, MDE, длительность, единицу анализа, доли трафика.
...
def n_per_group_binary(p_baseline: float, mde_rel: float, alpha: float, power: float, ratio: float = 1.0):
    p_alt = p_baseline * (1.0 + mde_rel)
    effect_size = proportion_effectsize(p_baseline, p_alt)
    analysis = NormalIndPower()
    nA = analysis.solve_power(effect_size=effect_size, alpha=alpha, power=power, ratio=ratio)
    return math.ceil(nA), math.ceil(nA * ratio)


def n_per_group_continuous(sigma: float, mde_abs: float, alpha: float, power: float, ratio: float = 1.0):
    d = mde_abs / sigma
    analysis = TTestIndPower()
    nA = analysis.solve_power(effect_size=d, alpha=alpha, power=power, ratio=ratio)
    return math.ceil(nA), math.ceil(nA * ratio)


def estimate_days(nA: int, nB: int, traffic: int, wA: float = 0.5, wB: float = 0.5):
    days_A = nA / (traffic * wA)
    days_B = nB / (traffic * wB)
    return math.ceil(max(days_A, days_B))


def sensitivity_table_binary(p_baseline: float, alpha: float, power: float, mde_rel_values):
...
def main():
    ...
    if args.metric == "binary":
        ...
        nA, nB = n_per_group_binary(args.baseline, args.mde_rel, args.alpha, args.power, args.ratio)
    else:
        if args.mde_abs is None or args.sigma is None:
            raise ValueError("--mde_abs and --sigma required for continuous metric")
        nA, nB = n_per_group_continuous(args.sigma, args.mde_abs, args.alpha, args.power, args.ratio)
    ...
    if args.metric == "binary":
        ...
        sensitivity_table_binary(args.baseline, args.alpha, args.power, mde_range)
