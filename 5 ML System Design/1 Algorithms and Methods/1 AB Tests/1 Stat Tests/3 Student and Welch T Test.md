...
- Метрические метрики (доход, время, чек, длительность сессии и т.п.), где среднее — валидная характеристика.
...
- Ясно указывайте: тип теста (Student/Welch), дву-/односторонность, $\alpha$, предпосылки и как вы их проверяли.
- Сообщайте: $\bar x_1, \bar x_2$, разность, $p$-value, df, **95% CI**, и **эффект‑сайз** ($d/g$, при неравных дисперсиях — также Glass’s $\Delta$).
